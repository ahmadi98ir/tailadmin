...
const Sidebar = ({ className = '' }: SidebarProps) => {
  ...
  useEffect(() => {
    const clickHandler = ({ target }: MouseEvent) => {
      if (!sidebarRef.current?.contains(target as Node)) {
        closeSidebar();
      }
    };
  ...
  return (
    <aside
      ...
      className={`absolute left-0 top-0 z-9999 flex h-screen w-72.5 flex-col overflow-y-hidden bg-black duration-300 ease-linear dark:bg-boxdark lg:static lg:translate-x-0 ${
        sidebarOpen ? 'translate-x-0' : isRTL ? 'translate-x-72.5' : '-translate-x-72.5'
      } ${className}`}
    >
      {/* Sidebar Header */}
              ...
              <li>
                ...
                >
                  ...
                  <svg
                    className={`absolute right-4 top-1/2 -translate-y-1/2 fill-current ${
                      menuItems.dashboard && 'rotate-180'
                    }`}
                    width="20"
                    height="20"
                    viewBox="0 0 20 20"
                ...
                <div
                  className={`translate transform overflow-hidden ${
                    menuItems.dashboard ? 'block' : 'hidden'
                  }`}
                >
                  <ul className="mt-4 mb-5.5 flex flex-col gap-2.5 pl-6">
                    ...
                    <li>
                      <Link
                        ...
                        className={`group relative flex items-center gap-2.5 rounded-md px-4 font-medium text-bodydark2 duration-300 ease-in-out hover:text-white ${
                          pathname === '/dashboard/marketing' && 'text-white'
                        }`}
                      >
                        {t('marketing')}
                        <span className="absolute right-4 block rounded bg-primary px-2 py-1 text-xs font-medium text-white">
